...
def puzzle_to_P2(puzzle):
    return [[[x for x in range(1,10) if not any([x == puzzle[ii][j] for ii in range(9) if i != ii]) and not any([x == puzzle[i][jj] for jj in range(9) if j != jj]) and not any([x == puzzle[ii][jj] for ii in range(i-(i%3), i-(i%3)+3) for jj in range(j-(j%3), j-(j%3)+3)])] if puzzle[i][j] == 0 else [puzzle[i][j]] for j in range(9)] for i in range(9)]
