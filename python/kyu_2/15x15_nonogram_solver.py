...
class Nonogram:
    ...
    def fast_possibilities(self, clues, spaces):
        if len(clues) > spaces+1:
            return []
        elif len(clues) == 0:
            return ['.'*spaces]
        elif len(clues) == 1:
            return ['.'*i + 'X'*clues[0] + '.'*(spaces-i) for i in range(0, spaces+1)]
        else:
            results = []
            for i in range(0, spaces):
                praefix = '.'*i + 'X'*clues[0] + '.'
                postfix = self.fast_possibilities(clues[1:], spaces-i-1)
                results.extend(f"{praefix}{p}" for p in postfix)
    ...
    def possibilities(self, clues, sure=None):
        ...
        poss = list(set(self.fast_possibilities(clues, self.dim-sum(clues))))
        for x in range(len(poss)):
            for num in clues:
                poss[x] = poss[x].replace('0', 'X'*num, 1)
        if sure is not None:
            poss = list(filter(lambda p: all([sure[d] == ' ' or sure[d] == p[d] for d in range(self.dim)]), poss))
        return poss
    ...
    def update_rows_p(self, i, j, s):
        if self.rows_p[i] is not None:
            self.rows_p[i] = list(filter(lambda poss: poss[j] == s, self.rows_p[i]))
    ...
    def update_cols_p(self, i, j, s):
        if self.cols_p[j] is not None:
            self.cols_p[j] = list(filter(lambda poss: poss[i] == s, self.cols_p[j]))
